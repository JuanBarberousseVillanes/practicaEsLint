...
class Car {
  ...
  /**
     ...
     *
     * @param {number} cantidad - The amount by which to accelerate the car.
     */
  accelerate(cantidad) {
    if (cantidad < 0) {
      throw new Error('Amount should be a positive number.');
    }
    if (this.speed + cantidad > this.maxspeed) {
  ...
  /**
     ...
     *
     * @return {boolean} A boolean value representing if the car is moving
     * or not
     */
  isMoving() {
    return this.speed > 0;
